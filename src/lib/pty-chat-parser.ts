 ...
 *
 * Accepts raw PTY byte chunks from Tauri's onPtyOutput event stream and
 * produces a structured ChatMessage[] that downstream chat rendering
 * components can consume.
 *
 * Usage with Tauri:
 *   const parser = new PtyChatParser()
 *   parser.onMessage((msg) => setMessages(prev => updateOrAppend(prev, msg)))
 *   const unlisten = await onPtyOutput(sessionId, (event) => {
...
export class PtyChatParser {
   ...
   */
  // @ts-expect-error TS6133 — assigned but not read; retained for future idle-time diagnostics
