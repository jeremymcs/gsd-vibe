...
// ============================================================
// M011 — Preferences Editor hooks
// ============================================================

export const useGsd2GetPreferences = (projectId: string) =>
  useQuery({
    queryKey: queryKeys.gsd2Preferences(projectId),
    queryFn: () => api.gsd2GetPreferences(projectId),
    staleTime: 30_000,
    retry: false,
  });
...
export const useGsd2SavePreferences = () => {
  ...
  return useMutation({
    ...
    }) => api.gsd2SavePreferences(projectId, scope, payload),
    onSuccess: (_data, { projectId }) => {
      toast.success('Preferences saved');
      void qc.invalidateQueries({ queryKey: queryKeys.gsd2Preferences(projectId) });
    },
    onError: (err: unknown) => {
      toast.error(`Failed to save preferences: ${getErrorMessage(err)}`);
    },
  });
};

// ── Global preferences (no project context) ──────────────────────────────────

export const useGsd2GetGlobalPreferences = () =>
  useQuery({
    queryKey: ['gsd2', 'global-preferences'],
    queryFn: () => api.gsd2GetGlobalPreferences(),
    staleTime: 30_000,
    retry: false,
  });

export const useGsd2SaveGlobalPreferences = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      api.gsd2SaveGlobalPreferences(payload),
    onSuccess: () => {
      toast.success('Global preferences saved');
      void qc.invalidateQueries({ queryKey: ['gsd2', 'global-preferences'] });
    },
    onError: (err: unknown) => {
...
export const useGsd2ListModels = () =>
  useQuery({
    ...
    staleTime: 10 * 60 * 1000, // 10 min — model list doesn't change often
    retry: false,              // fail silently if gsd CLI not available
  });
...
export const useGithubRepoInfo = (projectPath: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubRepoInfo(projectPath),
...
export const useGithubPrs = (projectPath: string, state?: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubPrs(projectPath, state),
...
export const useGithubPrReviews = (projectPath: string, prNumber: number, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubPrReviews(projectPath, prNumber),
    queryFn: () => api.githubGetPrReviews(projectPath, prNumber),
    enabled: enabled && !!projectPath && prNumber > 0,
    staleTime: 60 * 1000,
    retry: false,
  });

export const useGithubIssues = (projectPath: string, state?: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubIssues(projectPath, state),
    queryFn: () => api.githubListIssues(projectPath, state),
    enabled: enabled && !!projectPath,
    staleTime: 60 * 1000,
    retry: false,
  });
...
export const useGithubCheckRuns = (projectPath: string, gitRef: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubCheckRuns(projectPath, gitRef),
    queryFn: () => api.githubListCheckRuns(projectPath, gitRef),
    enabled: enabled && !!projectPath && !!gitRef,
    staleTime: 30 * 1000,
    retry: false,
  });
...
export const useGithubReleases = (projectPath: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubReleases(projectPath),
...
export const useGithubNotifications = (projectPath: string, enabled = true) =>
  useQuery({
    queryKey: queryKeys.githubNotifications(projectPath),
...
export const useGithubCreatePr = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (vars: { projectPath: string; title: string; body: string; head: string; base: string; draft: boolean }) =>
      api.githubCreatePr(vars.projectPath, vars.title, vars.body, vars.head, vars.base, vars.draft),
    onSuccess: (_data, vars) => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.githubPrs(vars.projectPath) });
    },
  });
};
...
export const useGithubCreateIssue = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (vars: { projectPath: string; title: string; body: string; labels: string[]; assignees: string[] }) =>
      api.githubCreateIssue(vars.projectPath, vars.title, vars.body, vars.labels, vars.assignees),
    onSuccess: (_data, vars) => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.githubIssues(vars.projectPath) });
    },
  });
};

// ============================================================
// gsd2 missing wrappers — hooks
// ============================================================
...
export const useGsd2MergeWorktree = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (vars: { projectId: string; worktreeName: string }) =>
      api.gsd2MergeWorktree(vars.projectId, vars.worktreeName),
    onSuccess: (_data, vars) => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.gsd2Worktrees(vars.projectId) });
    },
  });
};

export const useGsd2CleanWorktrees = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (projectId: string) => api.gsd2CleanWorktrees(projectId),
    onSuccess: (_data, projectId) => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.gsd2Worktrees(projectId) });
    },
  });
...
export const useGsd2HeadlessStartWithModel = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (vars: { projectId: string; model: string }) =>
      api.gsd2HeadlessStartWithModel(vars.projectId, vars.model),
    onSuccess: (_data, vars) => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.gsd2HeadlessSession(vars.projectId) });
    },
  });
};
...
export const useGithubImportGhToken = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.githubTokenStatus() });
    },
  });
};
