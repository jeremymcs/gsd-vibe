...
### Live timer pattern for active unit display (M010)

Use `useEffect + setInterval` seeded from server-provided `started_at` timestamp rather than polling the backend. The timer computes elapsed time client-side from the initial server value, avoiding unnecessary IPC calls. Clear the interval on unmount.

### include_str! for template bundling in Tauri (M010)

Compile-time template bundling via `include_str!` macros is the idiomatic Rust approach for embedding file content. Templates become part of the binary — zero runtime file I/O, no missing-file errors, and the compiler validates paths exist at build time.

### Post-action data registration pattern (M010)

When a multi-step action (e.g., scaffold → import → navigate) needs the result ID from one step for the next, call the registration mutation inline in the handler (not in a useEffect). This captures the result ID synchronously so the UI can wire it into buttons immediately. If registration fails, degrade gracefully with a warning and fallback URL.
