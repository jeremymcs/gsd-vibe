...
### R120 — Replace the current import-only project wizard with a full creation flow. User selects a language/framework starter template and optionally a GSD planning template, names the project, picks a parent directory, and gets a fully scaffolded project.
- Class: core-capability
- Status: active
...
### R121 — Bundled templates for React/Vite/TS, Next.js, Node.js API (Express), Rust CLI, Rust Axum API, Python (FastAPI), Python CLI, Go, Svelte, Tauri, plus a Blank template. Each creates real project scaffolds with proper configs.
- Class: core-capability
- Status: active
...
### R122 — Optional GSD planning layer that seeds .gsd/ directory with PROJECT.md, REQUIREMENTS.md, and a CONTEXT-DRAFT.md with milestone blueprints appropriate for the project archetype (web app, CLI tool, API, library).
- Class: core-capability
- Status: active
...
### R123 — Each language template includes proper configuration: tsconfig/Cargo.toml/pyproject.toml, .gitignore, README.md, test setup, linter config, and basic project structure. Not hello-world.
- Class: quality-attribute
- Status: active
...
### R124 — Add TypeScript invoke wrappers in tauri.ts for create_new_project, finalize_project_creation, and check_project_path. Add TanStack Query mutation hooks in queries.ts.
- Class: core-capability
- Status: active
...
### R125 — Extend use-gsd-file-watcher.ts to invalidate GSD-2 query keys (history, visualizer, health, inspect, undoInfo, gitSummary, recoveryInfo, hooks) when gsd2-specific file changes are detected.
- Class: core-capability
- Status: active
...
### R127 — pnpm build exits 0 with zero TypeScript errors. pnpm test passes all 146+ existing tests. cargo check --lib passes. No regressions from new code.
- Class: operability
- Status: active
...
### R112 — Transform the 139-line metric-only dashboard into a full project pulse landing view with 6+ widget sections: current unit, live metrics, slice progress, activity feed, git status, health summary.
- Class: core-capability
- Status: validated
...
- Supporting slices: none
- Validation: Active unit card with live timer implemented in gsd2-dashboard-view.tsx. Shows current milestone/slice/task with elapsed time via useEffect+setInterval pattern.
- Notes: Combines best of gsd-2 web dashboard with VibeFlow's existing overview widgets

### R113 — Dashboard shows the current active unit (milestone/slice/task), slice progress bar with percentage, and task checklist with done/active/pending status icons.
- Class: core-capability
- Status: validated
...
- Primary owning slice: M010/S01
- Supporting slices: none
- Validation: Slice progress bar with percentage and task checklist with done/active/pending status icons implemented in gsd2-dashboard-view.tsx SliceProgressSection.
- Notes: Uses existing VisualizerData2 milestones tree for task/slice data

### R114 — Top-level metric cards showing total cost, tokens used, units executed, and elapsed duration. Values formatted with existing formatCost/formatTokenCount/formatDuration utilities.
- Class: core-capability
- Status: validated
...
- Primary owning slice: M010/S01
- Supporting slices: none
- Validation: 4 metric cards (cost, tokens, units, duration) implemented in gsd2-dashboard-view.tsx MetricCards using formatCost/formatTokenCount/formatDuration utilities.
- Notes: Data from useGsd2History hook (already exists)

### R115 — Phase breakdown (execution/completion/planning/research/reassessment) with horizontal bar charts. Top 5 models by cost with token counts.
- Class: core-capability
- Status: validated
...
- Primary owning slice: M010/S01
- Supporting slices: none
- Validation: Phase breakdown bars showing cost distribution implemented in gsd2-dashboard-view.tsx PhaseBreakdownSection with horizontal bar charts.
- Notes: Already implemented in current dashboard — carried forward and refined

### R116 — Recent activity section showing execution events with typed indicators (success, error, system), timestamps, and scrollable history.
- Class: core-capability
- Status: validated
...
- Primary owning slice: M010/S01
- Supporting slices: none
- Validation: Activity feed showing last 8 entries with typed indicators and timestamps implemented in gsd2-dashboard-view.tsx ActivityFeedSection.
- Notes: VibeFlow has ActivityFeed component in project overview — integrate into dashboard

### R117 — Git status widget showing current branch, clean/dirty state, ahead/behind counts. Health summary showing doctor pass/fail counts and last check time.
- Class: core-capability
- Status: validated
...
- Primary owning slice: M010/S01
- Supporting slices: none
- Validation: Git status widget showing branch/clean-dirty/ahead-behind and health summary with doctor pass/fail counts implemented in gsd2-dashboard-view.tsx GitStatusSection.
- Notes: Uses existing gsd2_get_git_summary and gsd2_get_health commands

### R118 — Strip or render raw markdown formatting (**bold**, `backtick`, etc.) in text that comes from the Rust backend. No raw markdown visible in dashboard, visualizer, changelog, or any view that displays titles or summaries.
- Class: quality-attribute
- Status: validated
...
- Supporting slices: none
- Validation: strip_markdown_inline helper implemented in src-tauri/src/commands/gsd2.rs and integrated into load_slice_changelog. Removes **, __, *, and backtick markers. All 4 unit tests pass. Frontend renders clean text without raw markdown formatting.
- Notes: Affects changelog one-liners, milestone titles, slice titles, task titles across visualizer and dashboard

### R119 — When SUMMARY.md frontmatter lacks a one_liner key, extract the one-liner from the bold line after the heading in the body text (the established GSD format). Strip markdown bold markers from the extracted text.
- Class: quality-attribute
- Status: validated
...
- Primary owning slice: M010/S02
- Supporting slices: none
- Validation: extract_one_liner_from_body function implemented to scan SUMMARY.md body for **bold line** after H1 heading. Strips markdown via strip_markdown_inline. Used as or_else fallback in load_slice_changelog when frontmatter one_liner key is missing. Matches GSD auto-mode write pattern. Unit tests pass.
- Notes: Rust parse_frontmatter reads fm.get("one_liner") which returns None for auto-mode summaries

### R126 — Audit all list and data views. Every view that can show an empty dataset must display a contextual message with an icon and guidance, using the ViewEmpty pattern from shared/loading-states.tsx.
- Class: quality-attribute
- Status: validated
...
| R112 | core-capability | validated | M010/S01 | none | Active unit card with live timer implemented in gsd2-dashboard-view.tsx. Shows current milestone/slice/task with elapsed time via useEffect+setInterval pattern. |
| R113 | core-capability | validated | M010/S01 | none | Slice progress bar with percentage and task checklist with done/active/pending status icons implemented in gsd2-dashboard-view.tsx SliceProgressSection. |
| R114 | core-capability | validated | M010/S01 | none | 4 metric cards (cost, tokens, units, duration) implemented in gsd2-dashboard-view.tsx MetricCards using formatCost/formatTokenCount/formatDuration utilities. |
| R115 | core-capability | validated | M010/S01 | none | Phase breakdown bars showing cost distribution implemented in gsd2-dashboard-view.tsx PhaseBreakdownSection with horizontal bar charts. |
| R116 | core-capability | validated | M010/S01 | none | Activity feed showing last 8 entries with typed indicators and timestamps implemented in gsd2-dashboard-view.tsx ActivityFeedSection. |
| R117 | core-capability | validated | M010/S01 | none | Git status widget showing branch/clean-dirty/ahead-behind and health summary with doctor pass/fail counts implemented in gsd2-dashboard-view.tsx GitStatusSection. |
| R118 | quality-attribute | validated | M010/S02 | none | strip_markdown_inline helper implemented in src-tauri/src/commands/gsd2.rs and integrated into load_slice_changelog. Removes **, __, *, and backtick markers. All 4 unit tests pass. Frontend renders clean text without raw markdown formatting. |
| R119 | quality-attribute | validated | M010/S02 | none | extract_one_liner_from_body function implemented to scan SUMMARY.md body for **bold line** after H1 heading. Strips markdown via strip_markdown_inline. Used as or_else fallback in load_slice_changelog when frontmatter one_liner key is missing. Matches GSD auto-mode write pattern. Unit tests pass. |
